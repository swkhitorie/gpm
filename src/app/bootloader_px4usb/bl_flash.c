...
static uint8_t get_status(uint8_t bank_x)
{
	...
	if (status_reg & (1 << 17))			return 1;	//WRPERR=1,Ð´±£»¤´íÎó
	else if (status_reg & (1 << 18))	return 2;	//PGSERR=1,±à³ÌÐòÁÐ´íÎó
	else if (status_reg & (1 << 19))	return 3;	//STRBERR=1,¸´Ð´´íÎó 
	else if (status_reg & (1 << 21))	return 4;	//INCERR=1,Êý¾ÝÒ»ÖÂÐÔ´íÎó
	else if (status_reg & (1 << 22))	return 5;	//OPERR=1,Ð´/²Á³ý´íÎó 
	else if (status_reg & (1 << 23))	return 6;	//RDPERR=1,¶Á±£»¤´íÎó
	else if (status_reg & (1 << 24))	return 7;	//RDSERR=1,·Ç·¨·ÃÎÊ¼ÓÃÜÇø´íÎó 
	else if (status_reg & (1 << 25))	return 8;	//SNECCERR=1,1bit eccÐ£Õý´íÎó 
