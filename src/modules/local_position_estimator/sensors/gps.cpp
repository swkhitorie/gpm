...
#include <mavlink/mavlink_vehicle.h>
/*
    移植修改部分
        uorb sub 和 pub的对象修改
        适配已经移植的geo库
...
void BlockLocalPositionEstimator::gpsInit()
{
	...
	uint8_t nSat = _sub_gps->satellites_used;
	float eph = _sub_gps->eph;
	float epv = _sub_gps->epv;
	uint8_t fix_type = _sub_gps->fix_type;
...
//        if (++cnt > 20) {
//            cnt = 1;
//            mavlink_log_info(MAVLINK_COMM_1, "[lpe] gps reset %d %.3f, %.3f, %d ", nSat, eph, epv, fix_type);
//        }
		_gpsStats.reset();
		return;
	}
	...
	if (_gpsStats.getCount() > REQ_GPS_INIT_COUNT) {
		...
		if (!_receivedGps) {
			...
			if (!_map_ref.isInitialized()) {
				...
				_map_ref.initReference(gpsLat, gpsLon, hrt_absolute_time());
				// find origin
				_map_ref.reproject(-_x(X_x), -_x(X_y), gpsLatOrigin, gpsLonOrigin);
				// reinit origin
				_map_ref.initReference(gpsLatOrigin, gpsLonOrigin, hrt_absolute_time());
				...
				mavlink_log_info(MAVLINK_COMM_1, "[lpe] global origin init (gps) %d--> ", _map_ref.isInitialized());
				mavlink_log_info(MAVLINK_COMM_1, "lat %.8f lon %.8f alt %5.1f m",
...
int BlockLocalPositionEstimator::gpsMeasure(Vector<double, n_y_gps> &y)
{
	// gps measurement
	y.setZero();
	y(0) = _sub_gps->latitude_deg;
	y(1) = _sub_gps->longitude_deg;
	y(2) = _sub_gps->altitude_msl_m;
	y(3) = (double)_sub_gps->vel_n_m_s;
	y(4) = (double)_sub_gps->vel_e_m_s;
	y(5) = (double)_sub_gps->vel_d_m_s;
...
void BlockLocalPositionEstimator::gpsCorrect()
{
	...
	if (_sub_gps->eph > _param_lpe_gps_xy.get()) {
		var_xy = _sub_gps->eph * _sub_gps->eph;
	}

	if (_sub_gps->epv > _param_lpe_gps_z.get()) {
		var_z = _sub_gps->epv * _sub_gps->epv;
	}

	float gps_s_stddev =  _sub_gps->s_variance_m_s;
	...
	// publish innovations
	_pub_innov.gps_hpos[0] = r(0);
	_pub_innov.gps_hpos[1] = r(1);
	_pub_innov.gps_vpos    = r(2);
	_pub_innov.gps_hvel[0] = r(3);
	_pub_innov.gps_hvel[1] = r(4);
	_pub_innov.gps_vvel    = r(5);

	// publish innovation variances
	_pub_innov_var.gps_hpos[0] = S(0, 0);
	_pub_innov_var.gps_hpos[1] = S(1, 1);
	_pub_innov_var.gps_vpos    = S(2, 2);
	_pub_innov_var.gps_hvel[0] = S(3, 3);
	_pub_innov_var.gps_hvel[1] = S(4, 4);
	_pub_innov_var.gps_vvel    = S(5, 5);
	...
	if (beta / BETA_TABLE[n_y_gps] > beta_thresh) {
		if (!(_sensorFault & SENSOR_GPS)) {
			mavlink_log_critical(MAVLINK_COMM_1, "[lpe] gps fault %3g %3g %3g %3g %3g %3g",
		...
		_sensorFault &= ~SENSOR_GPS;
		mavlink_log_info(MAVLINK_COMM_1, "[lpe] GPS OK");
	}
...
void BlockLocalPositionEstimator::gpsCheckTimeout()
{
	if (_timeStamp - _time_last_gps > GPS_TIMEOUT) {
		if (!(_sensorTimeout & SENSOR_GPS)) {
			_sensorTimeout |= SENSOR_GPS;
			_gpsStats.reset();
			mavlink_log_critical(MAVLINK_COMM_1, "[lpe] GPS timeout ");
		}
	}
}
