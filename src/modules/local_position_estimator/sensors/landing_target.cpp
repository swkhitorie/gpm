...
int BlockLocalPositionEstimator::landingTargetMeasure(Vector<float, n_y_target> &y)
{
	if (_param_ltest_mode.get() == Target_Stationary) {
		if (_sub_landing_target_pose->rel_vel_valid) {
			y(0) = _sub_landing_target_pose->vx_rel;
			y(1) = _sub_landing_target_pose->vy_rel;
...
void BlockLocalPositionEstimator::landingTargetCorrect()
{
	...
	float cov_vx = _sub_landing_target_pose->cov_vx_rel;
	float cov_vy = _sub_landing_target_pose->cov_vy_rel;
