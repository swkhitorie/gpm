...
{
	matrix::Eulerf euler(matrix::Quatf(_sub_att->q));
	...
	// check for agl
	if (agl() < _sub_flow->min_ground_distance) {
		return -1;
	}

	// check quality
	float qual = _sub_flow->quality;
	...
	float flow_x_rad = _sub_flow->pixel_flow[0] * _param_lpe_flw_scale.get();
	float flow_y_rad = _sub_flow->pixel_flow[1] * _param_lpe_flw_scale.get();
	float dt_flow = _sub_flow->integration_timespan_us / 1.0e6f;
	...
	if (_param_lpe_fusion.get() & FUSE_FLOW_GYRO_COMP) {
		gyro_x_rad = _flow_gyro_x_high_pass.update(_sub_flow->delta_angle[0]);
		gyro_y_rad = _flow_gyro_y_high_pass.update(_sub_flow->delta_angle[1]);
	}
	...
	_flowQStats.update(Scalarf(_sub_flow->quality));
...
void BlockLocalPositionEstimator::flowCorrect()
{
	...
	const Vector3f rates{_sub_angular_velocity->xyz};
	...
	// publish innovations
	_pub_innov.flow[0] = r(0);
	_pub_innov.flow[1] = r(1);
	_pub_innov_var.flow[0] = S(0, 0);
	_pub_innov_var.flow[1] = S(1, 1);
