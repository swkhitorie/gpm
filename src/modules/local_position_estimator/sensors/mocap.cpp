...
{
	if (PX4_ISFINITE(_sub_mocap_odom->position_variance[0])) {
		// check if the mocap data is valid based on the variances
		_mocap_eph = sqrtf(fmaxf(_sub_mocap_odom->position_variance[0], _sub_mocap_odom->position_variance[1]));
		_mocap_epv = sqrtf(_sub_mocap_odom->position_variance[2]);
	...
	if (!_mocap_xy_valid || !_mocap_z_valid) {
		_time_last_mocap = _sub_mocap_odom->timestamp_sample;
		return -1;

	} else {
		_time_last_mocap = _sub_mocap_odom->timestamp_sample;

		if (PX4_ISFINITE(_sub_mocap_odom->position[0])) {
			y.setZero();
			y(Y_mocap_x) = _sub_mocap_odom->position[0];
			y(Y_mocap_y) = _sub_mocap_odom->position[1];
			y(Y_mocap_z) = _sub_mocap_odom->position[2];
...
//				 (double)_mocap_epv);
	...
	// publish innovations
	_pub_innov.ev_hpos[0] = r(0);
	_pub_innov.ev_hpos[1] = r(1);
	_pub_innov.ev_vpos    = r(2);
